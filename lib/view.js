...
function read_votes (joke_id) { 
	$.ajax("json/read_votes.php", {
		...
		success: function (data) {
			...
			if (data.like_own == 1) $("#like_link a").addClass("like"); else $("#like_link a").removeClass("like");
			$("#dislikes_sum").text(data.dislikes);
			$("#dislike_link").off("click").on("click", function() { send_vote(joke_id, "dislike"); });
			if (data.dislike_own == 1) $("#dislike_link a").addClass("dislike"); else $("#dislike_link a").removeClass("dislike");
			$("#loves_sum").text(data.loves);
			$("#love_link").off("click").on("click", function() { send_vote(joke_id, "love") });
			if (data.love_own == 1) $("#love_link a").addClass("love"); else $("#love_link a").removeClass("love");
