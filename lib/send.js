...
function send_vote(joke_id, action) {
	...
	$.ajax("vote2.php", {
		...
		success: function (data) {
			$("#vote_result").stop().text(data.message).slideDown(sleed).delay(5000).slideUp(sleed);
			target = "#" + action + "_link";
		...
		beforeSend: function () {
			$("#vote_result").stop().text("Submitting vote...").slideUp(sleed);
		},
		error: function () {
			$("#vote_result").text("Votes unavailable. We apologize.").slideDown(sleed).delay(5000).slideUp(sleed);
		},
		timeout: 15000,
	});
}
